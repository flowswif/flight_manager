...
document.getElementById('aircraft').addEventListener('change', function() {
    ...
    if (aircraftName) {
        fetch(`/get_aircraft_info?aircraft=${encodeURIComponent(aircraftName)}`)
            .then(response => response.json())
            .then(data => {
                console.log(data);
                if (data.aircraft_type) {
                    document.getElementById('aircraft_type').value = data.aircraft_type;
                    document.getElementById('crew_count').value = data.crew_count;
                    document.getElementById('max_takeoff_weight_t').value = data.max_takeoff_weight_t;
                    document.getElementById('max_landing_weight_t').value = data.max_landing_weight_t;
                    document.getElementById('empty_weight_t').value = data.empty_weight_t;
                    document.getElementById('total_fuel_weight_t').value = data.total_fuel_weight_t;
                } else {
                    console.warn("Пустой ответ, возможно ВС нет в базе");
                }
            })
            .catch(error => console.error('Ошибка получения данных о ВС:', error));
    }
});
